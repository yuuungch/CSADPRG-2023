...
fun getDeductions(salary: Double): Double {
...
fun main() {
    ...
    // Compute taxes
    var taxableIncome = salary - getDeductions(salary)
    var incomeTax = computeTax(taxableIncome)
    var afterIncomeTax = salary - incomeTax

    // Compute contributions
    var sss = computeSSS(salary)
    var philHealth = computePhilHealth(salary)
    var pagIbig = computePagIbig(salary)
    var totalContributions = getDeductions(salary)

    // Compute deductions
    var totalDeductions = incomeTax + totalContributions
    var netPay = salary - totalDeductions
    
    // Display output
    println("Income Tax: Php $incomeTax")
    println("Net Pay after Tax: Php $afterIncomeTax\n")

    println("SSS Contribution: Php $sss")
    println("PhilHealth Contribution: Php $philHealth")
    println("Pag-IBIG Contribution: Php $pagIbig")
    println("Total Contributions: $totalContributions\n")

    println("Total Deductions: Php $totalDeductions")
    println("Net Pay after Deductions: Php $netPay")
}